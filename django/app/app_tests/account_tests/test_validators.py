...
class TestCustomPasswordValidator:
  ...
  def test_valid_patterns(self, password):
  ...
  def test_invalid_patterns(self, password):
    validator = validators.CustomPasswordValidator()
...
@pytest.mark.account
@pytest.mark.validator
class TestCustomDigestValidator:
  def test_check_valid_digest(self):
    exact_val = 'abc123'
...
class TestCustomRegistrationTokenValidator:
  def test_check_valid_token(self, mocker):
  ...
  def test_invalid_token(self, mocker, config_indices):
    class DummyUser:
      is_active = True
