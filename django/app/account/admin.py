...
class CustomUserChangeForm(UserChangeForm):
  class Meta:
    model = User
    fields =  ('email', 'screen_name', 'password', 'is_staff', 'is_superuser')
...
class CustomUserAdmin(UserAdmin):
  ...
  add_fieldsets = (
    (None, {
      'classes': ('wide',),
      'fields': ('email', 'password1', 'password2', 'screen_name',),
    }),
  )

  form = CustomUserChangeForm
  add_form = CustomUserCreationForm
  list_display = ('email', 'screen_name', 'is_active', 'is_staff', 'is_superuser')
  list_filter = ('is_staff', 'is_superuser', 'is_active', 'groups')
  search_fields = ('email', 'screen_name')
  ordering = ('id', 'email', 'screen_name')
