...
module OpenSSL
  ...
  def self.self_sign(domain)
    ...
    command = <<-EOC
    openssl req -x509 \
      -CA #{domain.signed_cert_path} \
      -CAkey #{domain.key_path} \
      -in #{domain.csr_path} \
  ...
  def self.generate_dummy_certificate(dir, out_path, keyout_path)
    puts "Generating dummy certificate (root CA) for default fallback server"
    puts "Note: This certificate has the role as root CA."

    command = <<-EOC
      mkdir -p #{dir} && \
      openssl req -x509 -newkey \
        rsa:#{NAConfig.key_length} -noenc \
        -out #{out_path} \
        -keyout #{keyout_path} \
        -days 3650 \
        -batch \
